...
.PHONY: help init net certs seal-key up down restart logs ps status pull config \
	...
	runner-shell runner-env check-runner-env
...
check-runner-env:
	@test -f .runner.env || { \
		echo "make: .runner.env not found -- run 'make runner-env' and paste a PAT that may register runners on this repo (see .env.example)" >&2; \
		exit 1; \
	}
